...
import static xyz.jphil.persistent_storage.pfs.PFile.isWhiteList;
import static xyz.jphil.persistent_storage.pfs.PFile.isWhiteListAr;
import xyz.jphil.persistent_storage.pfs.type.TypeHandler;
 ...
 */
final class Copier {
    static void overwrite(PFs dest, Class t,Object put,TypeHandlerProvider thp,DefaultValue dv){
        if(!t.isInterface()){
    ...
    static void merge(Method method,PFs dest, Class t, Object put,TypeHandlerProvider thp){
        try{
    ...
    static void copy(PFs src, PFs dest,Class t){
        if(!t.isInterface()){
            throw new IllegalStateException("Only interfaces supported " + t);
        }  
        Object sobj = PFile.I(src, t);
        Object dobj = PFile.I(dest, t);
        Method[]ms=t.getMethods();
    ...
    private static void setProper(PFs dest,Method method,Object v,
            TypeHandlerProvider thp) throws Exception{
        Class cz = method.getReturnType();
        
        if(isWhiteList(cz)){
            PFile.set(dest, method.getName(), v);
        }else {
            // it is a PFile
            PFs r2 = dest.r(method.getName());
    ...
    private static void setProperArray(PFs dest,Method method,Object v,
            TypeHandlerProvider thp) throws Exception{
        Class cz = method.getReturnType();
        ...
        if(isWhiteListAr(cz)){
            ...
            if(ax.length>0){
                a = a+ax[ax.length - 1];
                PFile.set(dest, method.getName(), a);
            }
        }else {

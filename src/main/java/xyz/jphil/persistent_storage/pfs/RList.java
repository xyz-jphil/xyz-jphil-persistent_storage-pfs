...
public class RList implements List{
    ...
    public RList(PFs r, DefaultValue dv, TypeHandlerProvider thp) {
        this.r = r;
        this.dv = dv;
        this.thp = thp;
        
        try (DirectoryStream<Path> ds = Files.newDirectoryStream(((PFsImpl)r).p)){
            for(Path p : ds){
                String n = p.getFileName().toString();
                if(n.contains("."))continue;
                if(Files.isDirectory(p)){
                    hm.put(PFile.I(r, dv.subElementType(),thp),p);
                }else {
                    hm.put(PFile.get(r, n).o(dv, r, n, thp),p);
                }
    ...
    private Path makeRandomNew(){
        ...
        while(p==null || Files.exists(p)){
            p = ((PFsImpl)r).p.resolve((int)(Math.random()*Integer.MAX_VALUE)+"");
        } return p;
    }
